	...
	for i := 0; i < N; i++ {
		go func(i int) {
			...
			iter, _, err := c.newIters(
				&fileMetadata{fileNum: uint64(fileNum)}, nil /* compactionBounds */)
			if err != nil {
...
func TestTableCacheFrequentlyUsed(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		for _, j := range [...]int{pinned0, i % tableCacheTestNumTables, pinned1} {
			iter, _, err := c.newIters(
				&fileMetadata{fileNum: uint64(j)}, nil /* compactionBounds */)
			if err != nil {
...
func TestTableCacheIterLeak(t *testing.T) {
	...
	if _, _, err := c.newIters(&fileMetadata{fileNum: 0}, nil /* compactionBounds */); err != nil {
		t.Fatal(err)
	}
